      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
        ...
        <div>
          ...
          <div style="display: flex; flex-wrap: wrap; gap: 4px;">
            ${(data.tech_stack || ["Next.js 16", "TypeScript", "Tailwind CSS", "Supabase PostgreSQL", "Gemini 2.5 Pro"])
              .map(
                (tech: string) => `
              <span style="font-size: 10px; background: #e0e7ff; color: #4338ca; padding: 2px 8px; border-radius: 4px; font-weight: 600;">${tech}</span>
            `
              )
              .join("")}
          </div>
