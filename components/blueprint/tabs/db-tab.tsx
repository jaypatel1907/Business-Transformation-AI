...
export function DbTab({ generated, data }: { generated: boolean; data?: any }) {
  ...
  const dbEngine = data?.tech_stack?.database || "PostgreSQL / Supabase"
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
            ...
            Database ER Schema ({dbEngine})
          </h3>
          <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[10px] font-mono text-slate-600 border border-slate-200 font-semibold">
            PostgreSQL Schema
          </span>
        </div>
        ...
        <div className="space-y-4">
          {tables.map((table: any, idx: number) => (
            <div key={idx} className="rounded-xl border border-slate-200 bg-slate-50/60 overflow-hidden text-xs shadow-2xs">
              <div className="bg-slate-100 px-3.5 py-2.5 font-bold text-emerald-700 border-b border-slate-200 font-mono flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-500 font-normal">Primary & Foreign Keys Mapped</span>
              </div>
            ...
            <div key={idx} className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2.5">
                <span className={`px-2 py-0.5 rounded-md font-mono font-bold text-[10px] ${
                  ...
                  ep.method === 'PUT' ? 'bg-amber-100 text-amber-800 border border-amber-300' :
